...
{
    public partial class StudentCountByInsProfession
    {
            ...
            _options = new ApexChartOptions<CountsByProfessionInstitutionModel>
            {
                ...
                Chart = new()
                {
                    Toolbar = new Toolbar
                    {
                        ...
                        {
                            Png = new ExportPng
                            { Filename = "Üst Kurum/Uzmanlık Alanına Bağlı Öğrenci Sayısı" },
                            Csv = new ExportCSV
                            {
                                Filename = "Üst Kurum/Uzmanlık Alanına Bağlı Öğrenci Sayısı",
        ...
        private async Task GetFieldNames()
        {
            try
            {
                var response = await StudentService.CountsByProfessionInstitution(Filter);
                if (response.Result)
                {
                    _chartLoaded = true;
