...
{
    public partial class ProgramCountByProfession
    {
        ...
        private List<ActivePassiveResponseModel> educationFieldCounts = new List<ActivePassiveResponseModel>();
        ...
        private async Task GetFieldNames()
        {
            try
            {
                var response = await ProgramService.GetFieldNamesCountForDashboard(Filter);
                if (response.Result)
                {
                    _chartLoaded = true;
