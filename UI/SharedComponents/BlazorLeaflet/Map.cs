...
using UI.SharedComponents.BlazorLeaflet.Utils;
...
using UI.SharedComponents.BlazorLeaflet.Exceptions;
...
namespace UI.SharedComponents.BlazorLeaflet
{
    public class Map
    {
        ...
        /// outside the view.
        /// </summary>
        public List<LatLng> MaxBounds { get; set; }
        ...
        public void UpdateLayer(Layer layer)
        {
            ...
            var oldLayer = _layers.FirstOrDefault(x=> x.Id == layer.Id);
            if (oldLayer != null)
            {
                
                var index = _layers.IndexOf(oldLayer);
                if (index >= 0)
                {
                    _layers[index] = layer;
                }
        ...
        public async Task SetMapOnModal(Guid modalId) => await LeafletInterops.SetMapOnModal(_jsRuntime, Id, modalId);
        /// <summary>
