...
namespace UI.Pages.Archive.Students.StudentDetail.Tabs
{
    public partial class StudentOverview
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            try
            {
                var response = await DocumentService.GetListByTypeByEntity(Student.Id, DocumentTypes.OsymResultDocument);
                if (response.Result)
                {
