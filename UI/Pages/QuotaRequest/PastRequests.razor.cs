...
namespace UI.Pages.QuotaRequest
{
    ...
    public partial class PastRequests
    {
        [Inject] public IJSRuntime JsRuntime { get; set; }
        [Inject] private IQuotaRequestService QuotaRequestService { get; set; }
        [Inject] public ISweetAlert SweetAlert { get; set; }
        [Inject] public NavigationManager NavigationManager { get; set; }

        private List<QuotaRequestResponseDTO> _quotaRequests;
        private FilterDTO _filter;
        private PaginationModel<QuotaRequestResponseDTO> _paginationModel;
        ...
        private async Task GetQuotaRequests()
        {
            ...
            _paginationModel = await QuotaRequestService.GetPaginateList(_filter);
            if (_paginationModel.Items != null)
            {
                _quotaRequests = _paginationModel.Items;
                StateHasChanged();
            }
            else
            {
        ...
        private async Task OnDeleteHandler(QuotaRequestResponseDTO quotaRequest)
        {
            ...
            if (confirm)
            {
                try
                {
                    await QuotaRequestService.Delete(quotaRequest.Id);
                    _quotaRequests.Remove(quotaRequest);
                    StateHasChanged();
