using AutoMapper;
using Microsoft.AspNetCore.Components;
using Microsoft.AspNetCore.Components.Forms;
using Microsoft.JSInterop;
using Shared.FilterModels.Base;
using Shared.RequestModels;
using Shared.ResponseModels;
using Shared.ResponseModels.Wrapper;
using Shared.Types;
using System;
using System.Collections.Generic;
using System.Threading.Tasks;
using UI.Helper;
using UI.Models;
using UI.Services;
using UI.SharedComponents.Components;

namespace UI.Pages.VisitModule.StandardCategory
{
    public partial class StandardCategories
    {
        [Inject] private IStandardCategoryService StandardCategoryService { get; set; }
        [Inject] public ISweetAlert SweetAlert { get; set; }
        [Inject] public IJSRuntime JsRuntime { get; set; }
        [Inject] public IMapper Mapper { get; set; }

        private List<StandardCategoryResponseDTO> _standardCategories;
        private StandardCategoryResponseDTO _standardCategory;
        private PaginationModel<StandardCategoryResponseDTO> _paginationModel;
        private bool _loading;
        private List<BreadCrumbLink> _links;
        private FilterDTO _filter;
        private EditContext _standardCategoryEc;
        private MyModal _standardCategoryModal;
        private bool _saving;

        protected override async Task OnInitializedAsync()
        {
            _filter = new FilterDTO()
            {
                Sort = new[]{new Sort()
                {
                    Dir = SortType.ASC,
                    Field = "Name"
                }},
                Filter = new()
                {
                    Logic = "and",
                    Filters = new()
                    {
                        new Filter()
                        {
                            Field = "IsDeleted",
                            Operator = "eq",
                            Value = false
                        },
                    }
                }
            };

            _links = new List<BreadCrumbLink>()
            {
                new BreadCrumbLink()
                    {
                        IsActive = true,
                        To = "/",
                        OrderIndex = 0,
                        Title = L["Homepage"]
                    },new BreadCrumbLink()
                    {
                        IsActive = false,
                        To = "/visit-module/standards",
                        OrderIndex = 1,
                        Title = L["_List", L["Standard"]]
                    }
            };
            await GetStandardCategories();
            await base.OnInitializedAsync();
        }

        private async Task GetStandardCategories()
        {
            if (_filter?.Filter?.Filters?.Count == 0)
            {
                _filter.Filter.Filters = null;
                _filter.Filter.Logic = null;
            }
            _paginationModel = await StandardCategoryService.GetPaginateList(_filter);
            if (_paginationModel.Items != null)
            {
                _standardCategories = _paginationModel.Items;
                StateHasChanged();
            }
            else
            {
                _loading = true;
                SweetAlert.ErrorAlert();
            }
        }
        private void OpenModalForAdd()
        {
            _standardCategory = new();
            _standardCategoryEc = new EditContext(_standardCategory);

            _standardCategoryModal.OpenModal();
        }
        private void OpenModalForEdit(StandardCategoryResponseDTO standard)
        {
            _standardCategory = standard;
            _standardCategoryEc = new EditContext(_standardCategory);

            _standardCategoryModal.OpenModal();
        }
       
        private async Task SaveStandard()
        {
            if (!_standardCategoryEc.Validate()) return;

            _saving = true;
            StateHasChanged();
            ResponseWrapper<StandardCategoryResponseDTO> response;
            try
            {
                if (_standardCategory.Id > 0)
                {
                    response = await StandardCategoryService.Update(_standardCategory.Id, Mapper.Map<StandardCategoryDTO>(_standardCategory));
                }
                else
                {
                    response = await StandardCategoryService.Add(Mapper.Map<StandardCategoryDTO>(_standardCategory));
                }

                if (response.Result)
                {
                    SweetAlert.ToastAlert(SweetAlertIcon.success, $"{L["Successfully Saved"]}");
                    _standardCategoryModal.CloseModal();
                }
                else
                {
                    throw new Exception(response.Message);
                }
            }
            catch (Exception e)
            {
                SweetAlert.ToastAlert(SweetAlertIcon.error, e.Message);
                Console.WriteLine(e.Message);
            }
            _saving = false;
            StateHasChanged();
            await GetStandardCategories();
        }
        private async Task OnDeleteHandler(StandardCategoryResponseDTO standard)
        {
            var confirm = await SweetAlert.ConfirmAlert($"{L["Are you sure?"]}",
            $"{L["Are you sure you want to delete this item? This action cannot be undone."]}",
            SweetAlertIcon.question, true, $"{L["Delete"]}", $"{L["Cancel"]}");

            if (confirm)
            {
                try
                {
                    await StandardCategoryService.Delete(standard.Id);
                    SweetAlert.ToastAlert(SweetAlertIcon.success, $"{L["Item Deleted!"]}");
                }
                catch (Exception e)
                {
                    Console.WriteLine(e);
                    SweetAlert.ErrorAlert();
                    throw;
                }

                await GetStandardCategories();

            }
        }

        #region Filter
        private async Task OnChangeFilter(ChangeEventArgs args, string filterName)
        {
            var value = (string)args.Value;
            _filter.Filter ??= new Filter();
            _filter.Filter.Filters ??= new List<Filter>();
            _filter.Filter.Logic ??= "and";
            _filter.page = 1;
            var index = _filter.Filter.Filters.FindIndex(x => x.Field == filterName);
            if (index < 0)
            {
                _filter.Filter.Filters.Add(new Filter()
                {
                    Field = filterName,
                    Operator = "contains",
                    Value = value
                });
            }
            else
            {
                _filter.Filter.Filters[index].Value = value;
            }
            await GetStandardCategories();
        }
        private async Task OnResetFilter(string filterName)
        {
            _filter.Filter ??= new Filter();
            _filter.Filter.Filters ??= new List<Filter>();
            _filter.Filter.Logic ??= "and";
            _filter.page = 1;
            var index = _filter.Filter.Filters.FindIndex(x => x.Field == filterName);
            if (index >= 0)
            {
                _filter.Filter.Filters.RemoveAt(index);
                await JsRuntime.InvokeVoidAsync("clearInput", filterName);
                await GetStandardCategories();
            }
        }
        private async Task OnSortChange(Sort sort)
        {
            _filter.Sort = new[] { sort };
            await GetStandardCategories();
        }
        private bool IsFiltered(string filterName)
        {
            _filter.Filter ??= new Filter();
            _filter.Filter.Filters ??= new List<Filter>();
            _filter.Filter.Logic ??= "and";
            var index = _filter.Filter.Filters.FindIndex(x => x.Field == filterName);
            return index >= 0;
        }
        private async Task PaginationHandler(PaginationInfo val)
        {
            var (item1, item2) = (val.Page, val.PageSize);

            _filter.page = item1;
            _filter.pageSize = item2;

            await GetStandardCategories();
        }

        #endregion
    }
}
