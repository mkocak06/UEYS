...
{
    public partial class CommitteeDoneRequests
    {
        ...
        protected override async Task OnInitializedAsync()
        {
            _links = new List<BreadCrumbLink>()
            {
                ...
                },new BreadCrumbLink()
                {
                    IsActive = false,
                    OrderIndex = 1,
                    Title = L["Yerinde Ziyareti Tamamlanan Talepler"]
                },
            };
                ...
                Filter = new()
                {
                    ...
                    Filters = new()
                    {
                        ...
                        new Filter()
                        {
                            ...
                            Operator = "eq",
                            Value = VisitStatusType.CommitteeEvaluated
                        },
                    }
                }
            };
        ...
        private async void OnCanceled(bool check)
        {
            if (_isReview) _isReview = false;
            _isEditing = false;
            StateHasChanged();
            if(!check) await GetVisitForms();
        }
