...
namespace UI.Pages.Management.SpecificEducationPlaces
{
    ...
    public partial class SpecificEducationPlaces
    {
        [Inject] private ISpecificEducationPlaceService SpecificEducationPlaceService { get; set; }
        [Inject] public ISweetAlert SweetAlert { get; set; }
        [Inject] public IJSRuntime JsRuntime { get; set; }
        [Inject] public NavigationManager NavigationManager { get; set; }

        private List<SpecificEducationPlaceResponseDTO> _specificEducationPlaces;
        private PaginationModel<SpecificEducationPlaceResponseDTO> _paginationModel;
            ...
            _links = new List<BreadCrumbLink>()
        {
            ...
            await GetSpecificEducationPlaces();
        ...
        private async Task GetSpecificEducationPlaces()
        {
            ...
            _paginationModel = await SpecificEducationPlaceService.GetPaginateList(_filter);
            if (_paginationModel.Items != null)
            {
                _specificEducationPlaces = _paginationModel.Items;
                StateHasChanged();
            }
            else
            {
        ...
        private async Task OnDeleteHandler(SpecificEducationPlaceResponseDTO specificEducationPlace)
        {
            ...
            if (confirm)
            {
                try
                {
                    await SpecificEducationPlaceService.Delete((long)specificEducationPlace.Id);
                    _specificEducationPlaces.Remove(specificEducationPlace);
                    StateHasChanged();
