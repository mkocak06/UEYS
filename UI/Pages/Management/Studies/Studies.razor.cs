...
namespace UI.Pages.Management.Studies
{
    ...
    {
        [Inject] private IStudyService StudyService { get; set; }
        [Inject] public ISweetAlert SweetAlert { get; set; }
        [Inject] public IJSRuntime JsRuntime { get; set; }
        [Inject] public NavigationManager NavigationManager { get; set; }

        private List<StudyResponseDTO> _studies;
        private PaginationModel<StudyResponseDTO> _paginationModel;
        ...
        private async Task GetStudies()
        {
            ...
            _paginationModel = await StudyService.GetPaginateList(_filter);
            if (_paginationModel.Items != null)
            {
                _studies = _paginationModel.Items;
                StateHasChanged();
            }
            else
            {
        ...
        private async Task OnDeleteHandler(StudyResponseDTO study)
        {
            ...
            if (confirm)
            {
                try
                {
                    await StudyService.Delete((long)study.Id);
