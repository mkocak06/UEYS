...

{
    ...
    public partial class StudentThesis
    {
        ...
        private MyModal _thesisDeleteModal;
        ...
        public string _deleteReasonValidationMessage;
        ...
        private async Task OnUpdateHandler(ThesisResponseDTO thesisResponse)
        {
            try
            {
                ...
                if (response.Result)
                {
                    _readOnly = response.Item.IsDeleted;
        ...
        {
            _thesis = thesisResponse;
            _thesisDeleteModal.OpenModal();
            _deleteReasonValidationMessage = "This field is required";
        }


        private async Task DeleteThesis()
        {
            if (string.IsNullOrWhiteSpace(_thesis.DeleteReasonExplanation))
                return;
            var confirm = await SweetAlert.ConfirmAlert($"{L["Are you sure?"]}",
                L["You are ending this thesis. You can access the thesis information but you cannot make any changes. Do not forget to add your new thesis. Do you want to proceed?"],
                SweetAlertIcon.question, true, L["Yes"], L["No"]);
            if (confirm)
            {
                try
                {
                    _thesis.IsDeleted = true;

                    var response = await ThesisService.Put(_thesis.Id.Value, Mapper.Map<ThesisDTO>(_thesis));
                    if (response.Result)
                    {
                        _thesisDeleteModal.CloseModal();
                        SweetAlert.ToastAlert(SweetAlertIcon.success, $"{L["Item Deleted!"]}");
                        await GetTheses();
                    }
                    else
                    {
                        _thesis.IsDeleted = false;
                        _thesis.DeleteReasonExplanation = null;
                        StateHasChanged();
