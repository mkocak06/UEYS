...
namespace UI.Pages.Student.Students.StudentDetail.Tabs
{
    public partial class Registration
    {
        ...
        private MyModal sendRegistrationModal;
        private Dropzone registrationDropzone;
        ...
        private void OnSendRegistrationModal()
        {
            registrationDropzone.ResetStatus();
            sendRegistrationModal.OpenModal();
        }
        ...
        public async Task<bool> CallDropzone()
        {
            ...
            try
            {
                var response = await registrationDropzone.SubmitFileAsync();
                if (response.Result)
                {
        ...
        private async void SendToRegistration()
        {
            ...
            if (string.IsNullOrEmpty(registrationDropzone._selectedFileName))
            {
                _documentValidatorMessage = "Dosya yükleme zorunludur!";
                StateHasChanged();
                return;
            }
            else
