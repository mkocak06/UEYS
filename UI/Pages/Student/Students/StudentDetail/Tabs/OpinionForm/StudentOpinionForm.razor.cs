...
namespace UI.Pages.Student.Students.StudentDetail.Tabs.OpinionForm
{
    public partial class StudentOpinionForm
    {
        ...
        private OpinionFormResponseDTO _opinion = new() { Documents = new() };
        ...
        public Dropzone dropzoneForAddCommunique;
        public Dropzone dropzoneForUpdate;
        public Dropzone dropzoneForUpdateCommunique;
        public MyModal UploaderModalForAdd;
        public MyModal UploaderModalForAdd_2;
        public MyModal UploaderModalForUpdate;
        public MyModal UploaderModalForUpdate_2;
        private bool _fileLoaded = true;
        public bool _saving;
        private int[] _scores;
        private string _explanationValidationMessage;
        private string _documentValidationMessage;
        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            //    }
            //    _opinion = new() { StartDate = _lastDate, EndDate = _lastDate?.AddMonths(6), StudentId = _student.Id, IsRepeating = IsExplanationRequired, Documents = new() };
            ...
            if (_opinion.Id == null)
                GetStartAndEndDates();
            var educationTrackings = await EducationTrackingService.GetTimeIncreasingRecordsByDate(new OpinionFormRequestDTO() { StudentId = _student.Id, StartDate = _opinion.StartDate, EndDate = _opinion.EndDate });
        ...
        public async void GetStartAndEndDates()
        {
            ...
            _opinion.StartDate = response?.Item.StartDate;
            _opinion.EndDate = response?.Item.EndDate;
            _opinion.Period = response?.Item.Period;
    ...
    public async Task<bool> CallDropzoneCommunique()
    {
        _fileLoaded = false;
        StateHasChanged();
        try
        {
            var result = await dropzoneForAddCommunique.SubmitFileAsync();
            if (result.Result)
            {
                _opinion.Documents ??= new();
                _opinion.Documents.Add(result.Item);
                StateHasChanged();
                UploaderModalForAdd_2.CloseModal();
                return true;
            }
            else
                return false;
        ...
        {
            _fileLoaded = true;
            dropzoneForAddCommunique.ResetStatus();
            StateHasChanged();
        }
    }
    ...
    public async Task<bool> CallDropzoneForUpdate()
    {
        try
        {
            ...
            if (result.Result)
            {
                ...
                _opinion.Documents.Add(result.Item);
                StateHasChanged();
                UploaderModalForUpdate.CloseModal();
                return true;
            }
            else
            {
    ...
    public async Task<bool> CallDropzoneForUpdateCommunique()
    {
        try
        {
            var result = await dropzoneForUpdateCommunique.SubmitFileAsync();
            if (result.Result)
            {
                _opinion.Documents ??= new();
                _opinion.Documents.Add(result.Item);
                StateHasChanged();
                UploaderModalForUpdate_2.CloseModal();
                return true;
            }
            else
                return false;
        ...
        finally { dropzoneForUpdateCommunique.ResetStatus(); }
    }
    ...
    public async Task Save()
    {
        ...
        _ec.GetValidationMessages().PrintJson("validation messages");
        ...
        try
        {
            ...
            else
            {
                ...
                if (response.Result)
                {
                    foreach (var item in _opinion.Documents)
                    {
