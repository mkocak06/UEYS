    ...
    public partial class StudentRotation
    {
        ...
        private async Task LoadRotations()
        {
            if (_student.CurriculumId != null)
            {
                ...
                _uncompletedStudentRotations.Clear();
                try
                {
        ...
        private async Task OnRemoveCompletedStudentRotationHandler(StudentRotationResponseDTO studentRotation)
        {
            ...
            if (confirm)
            {
                ...
                {
                    await SweetAlert.ConfirmAlert(L["Warning"], L["You are not authorized for this operation!"], SweetAlertIcon.error, false, "Tamam", "");
                    _saving = false;
                    StateHasChanged();
                    return;
                }
        ...
        private async Task OnRemoveUnCompletedStudentRotationHandler(StudentRotationResponseDTO studentRotation)
        {
            var confirm = await SweetAlert.ConfirmAlert($"{L["Are you sure?"]}", $"{L["Are you sure you want to delete this item? This action cannot be undone."]}", SweetAlertIcon.question, true, $"{L["Delete"]}", $"{L["Cancel"]}");
        ...
        private async Task OnStudentRotationDetail(StudentRotationResponseDTO studentRotation)
        {
            ...
            if (response.Result && response.Item != null)
            {
                ...
                _loaded = true;
                StateHasChanged();
                _studentRotationUpdateModel?.StudentRotationPerfections?.Count.PrintJson("srp");
                _rotations?.FirstOrDefault(x => x.RotationId == _studentRotationUpdateModel?.RotationId)?.Perfections?.Count.PrintJson("rot");
                _studentRotationDetailModal.OpenModal();
            }
        }
        ...
        private async Task FinishStudentRotation(bool isUncompleted)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    SweetAlert.ConfirmAlert(L["Warning"], L[response.Message], SweetAlertIcon.warning, false, L["Okey"], "");
                }
            }
            catch (Exception e)
            {
        ...
        private async void OnChangeBeginDate(DateTime? e)
        {
            _studentRotation.BeginDate = e;

            var studentRotationDTO = new StudentRotationDTO() { BeginDate = e, RotationId = _studentRotation.RotationId, StudentId = _student.Id };

            if (e != null)
            {
                var response = await StudentRotationService.GetEndDateByStartDate(studentRotationDTO);

                if (response.Result)
                {
                    _studentRotation.EndDate = response.Item.EndDate;
                    var educationTrackings = await EducationTrackingService.GetTimeIncreasingRecordsByDate(new OpinionFormRequestDTO() { StudentId = _student.Id, StartDate = _studentRotation.BeginDate, EndDate = _studentRotation.EndDate });
